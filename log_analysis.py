...
def get_pupular_authors():
    ...
    cursor.execute("select name ,count(name) as num  from articles_authors join log_remove_root_path_fist on articles_authors.slug =log_remove_root_path_fist.substring group by name")
    results = cursor.fetchall()
